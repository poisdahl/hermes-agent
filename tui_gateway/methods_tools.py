        ...
        def go(mgr, cwd):
            ...
            if result.get("success") and not file_path:
                ...
                with session["history_lock"]:
                    if session.get("running"):
                        raise RuntimeError(
                            "session became busy before rollback history could be rewound"
                        )
                    history = _history_without_ephemeral_scaffolding(
                        session.get("history", [])
                    )
                    ...
                    if user_indices:
                        installed, _live_view = history_before_user_originated_turn(
                            history, user_indices[-1]
                        )
                        removed = len(history) - user_indices[-1]
