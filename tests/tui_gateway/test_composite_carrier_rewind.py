...
def test_retry_fails_closed_when_same_target_has_a_different_warm_prefix(
    carrier_session,
):
    db, install = carrier_session
    sid, session_key, session = install(
        [
            {"role": "user", "content": "DURABLE PREFIX ASK"},
            {"role": "assistant", "content": "durable prefix answer"},
            _composite_carrier(),
            {"role": "assistant", "content": "failed"},
        ]
    )
    session["history"][0]["content"] = "WARM PREFIX ASK"
    session["history"][1]["content"] = "warm prefix answer"
    session["agent"]._session_messages = list(session["history"])
    ...
    assert "session history changed" in response["error"]["message"]
    assert session["history"] == before_history
    after_rows = db._conn.execute(
        "SELECT id, content, active FROM messages "
        "WHERE session_id = ? ORDER BY id",
        (session_key,),
    ).fetchall()
...
def test_retry_repairs_assistants_exposed_by_buried_ephemeral_scaffolding(
    carrier_session,
):
    db, install = carrier_session
    sid, session_key, session = install(
    ...
    assert response["result"] == {"type": "send", "message": "REAL ASK"}
    assert [message.get("content") for message in session["history"][:2]] == [
        "OLDER ASK",
        "candidate answer\nverified answer",
    ]
    active = db.get_messages_as_conversation(session_key, include_row_ids=True)
    ...
    assert session["history"][2]["display_kind"] == "hidden"
    assert "REAL ASK" not in session["history"][2]["content"]


def test_retry_normalizes_older_media_prefix_but_still_targets_plain_ask(
    carrier_session,
):
    db, install = carrier_session
    sid, session_key, session = install(
...
def test_history_projection_unwraps_composite_and_hides_sole_handoff():
    composite = _composite_carrier()
    sole_handoff = {
        **composite,
        "content": composite["content"].split("\n\nREAL ASK", 1)[0],
    }

    assert server._history_to_messages([composite]) == [
        {"role": "user", "text": "REAL ASK"}
    ]
    assert server._history_to_messages([sole_handoff]) == []


def test_retry_rejects_degraded_media_before_mutating_history(carrier_session):
    db, install = carrier_session
    carrier = _composite_carrier()
    carrier["content"] = carrier["content"].replace(
    ...
    assert "cannot safely reconstruct media" in response["error"]["message"]
...
def test_full_rollback_preserves_composite_scaffold(carrier_session):
    db, install = carrier_session
    sid, session_key, session = install(
        [_composite_carrier(), {"role": "assistant", "content": "failed"}]
    )
    ...
    class _CheckpointManager:
        ...
        def list_checkpoints(self, _cwd):
            return [{"hash": "abc123"}]

        def restore(self, _cwd, _target, file_path=None):
            assert file_path is None
            return {"success": True, "message": "restored"}

    session["agent"]._checkpoint_mgr = _CheckpointManager()
    before_rows = db._conn.execute(
        "SELECT id, content, active FROM messages "
        "WHERE session_id = ? ORDER BY id",
        (session_key,),
    ...
    response = server._methods["rollback.restore"](
        "request-id",
        {"session_id": sid, "hash": "abc123"},
    )

    assert response["result"]["success"] is True
    assert response["result"]["history_removed"] == 2
    assert len(session["history"]) == 1
    assert session["history"][0]["display_kind"] == "hidden"
    assert "REAL ASK" not in session["history"][0]["content"]
    after_rows = db._conn.execute(
        "SELECT id, content, active FROM messages "
        "WHERE session_id = ? ORDER BY id",
        (session_key,),
