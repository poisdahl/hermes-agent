...
def test_personality_marker_does_not_shift_truncate_ordinal(monkeypatch):
    ...
    try:
        ...
        stub_db = _SoftRewindDB(history_before)
        ...
        assert stub_db.rewinds == [("session-key", third_index + 1, False)], (
            "the pivot shifted the ordinal: the cut landed at "
            f"row {stub_db.rewinds[0][1] if stub_db.rewinds else None} "
            f"instead of {third_index + 1}"
        )
        assert [
            {key: value for key, value in message.items() if key != "_row_id"}
            for message in stub_db.active
        ] == expected
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    unrecoverable for the same reason: the old `replace_messages` path DELETEd
    the rows and the FTS entry with them. The handler now delegates to the
    transactionally soft-deleting `rewind_to_message` primitive. Its storage
    ...
    try:
        ...
        assert resp.get("result"), f"got error: {resp.get('error')}"
        assert stub_db.rewinds == [("session-key", 3, False)], (
            "a rewind must use the soft-delete primitive, not replace rows"
        )
        assert [message["content"] for message in stub_db.active] == [
            "first",
            "first reply",
        ]
    finally:
