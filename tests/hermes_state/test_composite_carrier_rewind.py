...
def test_composite_rewind_restore_archives_its_replacement_scaffold(db):
    sid = "carrier-rewind-restore"
    db.create_session(sid, source="tui")
    target_id = db.append_message(sid, "user", _carrier())
    tail_id = db.append_message(sid, "assistant", "failed")
    _, revision = db.get_active_conversation_snapshot(sid)
    ...
    assert restored == 2
    active = db.get_messages_as_conversation(sid, include_row_ids=True)
    assert [message["_row_id"] for message in active] == [target_id, tail_id]
    replacement = db._conn.execute(
        "SELECT active FROM messages WHERE id = ?",
        (result["replacement_message_id"],),
    ).fetchone()
    assert replacement["active"] == 0
    assert _session_counts(db, sid) == (2, 0, 1)


def test_composite_restore_rejects_wrong_replacement_atomically(db):
    sid = "carrier-rewind-restore-wrong-id"
    db.create_session(sid, source="tui")
    target_id = db.append_message(sid, "user", _carrier())
    db.append_message(sid, "assistant", "failed")
    result = db.rewind_to_message(
    ...
    with pytest.raises(ValueError, match="replacement_message_id"):
        db.restore_rewound(
            sid,
            target_id,
            replacement_message_id=target_id,
        )

    assert _row_state(db, sid) == before_rows
    assert _session_counts(db, sid) == before_counts
    assert result["replacement_message_id"] != target_id


def test_composite_restore_rejects_unrelated_active_hidden_row(db):
    sid = "carrier-rewind-restore-unrelated-hidden"
    db.create_session(sid, source="tui")
    target_id = db.append_message(sid, "user", _carrier())
    db.append_message(sid, "assistant", "failed")
    result = db.rewind_to_message(
    ...
    unrelated_id = db.append_message(
        sid,
        "user",
        "unrelated hidden timeline row",
        display_kind="hidden",
    )
    before_rows = _row_state(db, sid)
    before_counts = _session_counts(db, sid)
    ...
    with pytest.raises(ValueError, match="replacement_message_id"):
        db.restore_rewound(
            sid,
            target_id,
            replacement_message_id=unrelated_id,
        )
    ...
    assert result["replacement_message_id"] in active_ids
    assert unrelated_id in active_ids
