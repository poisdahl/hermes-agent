...
def retryable_user_text(content: Any) -> str:
    ...
    Retry has no attachment replay protocol.  Media, unknown structured
    parts, and degraded media placeholders therefore fail closed.  Structured
    content is flattened only when every part is plain text.
    """
    if isinstance(content, str):
        text = content
    ...
    if _RETRY_MEDIA_PLACEHOLDER_RE.search(text):
        raise ValueError("retry cannot safely reconstruct media from stored history")
    return text


def summary_carrier_persistence_display(
    message: Any,
) -> tuple[Optional[str], Optional[Dict[str, Any]]]:
    """Return durable display fields for a compaction-summary carrier.

    A pure handoff is hidden scaffolding.  An untyped composite carrier stays
    visible so older clients do not lose its embedded human ask.  Explicitly
    typed or hidden user rows retain their declared presentation.  Non-user
    carriers preserve the historical flush policy: standalone summaries (or
    in-process summaries explicitly marked as lacking user-turn provenance)
    are hidden, while cold heuristic-only merged carriers retain their existing
    presentation.  The helper is side-effect-free.
    """
    if not isinstance(message, dict):
        return None, None
    display_kind = message.get("display_kind")
    ...
    if isinstance(display_metadata, dict):
        metadata = copy.deepcopy(display_metadata)
    elif isinstance(display_metadata, str):
        try:
            decoded_metadata = json.loads(display_metadata)
        except (json.JSONDecodeError, TypeError):
    ...
    if message.get("role") != "user":
        if (
            ...
            or (
                ...
                and not message.get(COMPRESSED_SUMMARY_HAS_USER_TURN_KEY)
            )
        ):
